    ...
    <div class="sm:rounded-1rem">
      ...
      <div class="flex blog-content">
        ...
        <div class="flex-1 overflow-hidden" @click="gotoSteem($event)">
          ...
          <!--       foreign page -->
<!--          <LinkPreview @click.stop="clickLinkView()" class="cursor-pointer mt-10px"-->
<!--                       v-if="post.pageInfo && post.pageInfo.length>10 && !isIgnoreAccount"-->
...
<!--          <Repost class="mt-10px" :content-class="contentClass" @click.stop="clickRetweetView()" v-if="post.retweetInfo && post.retweetInfo.length>10 && !isIgnoreAccount" :retweetInfo="post.retweetInfo"/>-->
          ...
          <div class="grid mt-10px md:max-w-35rem rounded-12px overflow-hidden border-1 border-listBgBorder img-1"
               v-if="imgurls && imgurls.length > 0">
            <div class="img-box" v-for="(url, index) of imgurls.slice(0,1)" :key="url">
              <img @click.stop="viewImg(index)" :src="url" alt="">
            </div>
          </div>
          <div v-else-if="post.pageInfo && post.pageInfo.length>10 && !isIgnoreAccount"
               class="grid mt-10px md:max-w-35rem rounded-12px overflow-hidden border-1 border-listBgBorder img-1">
            <div class="img-box">
