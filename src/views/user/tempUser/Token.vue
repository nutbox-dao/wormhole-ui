  ...
  <div class="pb-2rem">
    <div class="flex justify-between items-center py-1rem px-1.5rem border-b-1 border-listBgBorder"
      ...
      <div class="flex items-center">
        <img class="w-2rem h-2rem rounded-full border-1px gradient-border"
             :src="icons[erc20]" alt="">
        <div class="text-left ml-1rem">
          <div class="c-text-black text-1rem">{{names[erc20]}}</div>
          <div class="text-text8F text-0.75rem font-bold mt-0.5rem">{{ formatAmount(erc20Balances.ETH[erc20]) }} {{erc20}}</div>
        </div>
      </div>
      <div class="flex flex-col items-end justify-end">
        <div class="text-1rem text-primaryColor c-text-black">${{formatAmount(erc20Balances.ETH[erc20] * prices[erc20.toLowerCase()])}}</div>
      </div>
    </div>
    ...
    <div class="flex justify-between items-center py-1rem px-1.5rem border-b-1 border-listBgBorder"
      ...
      <div class="flex items-center">
        <img class="w-2rem h-2rem rounded-full border-1px gradient-border"
             :src="icons[erc20]" alt="">
        <div class="text-left ml-1rem">
          <div class="c-text-black text-1rem">{{names[erc20]}}(BSC)</div>
          <div class="text-text8F text-0.75rem font-bold mt-0.5rem">{{ formatAmount(erc20Balances.BNB[erc20]) }} {{erc20}}</div>
        </div>
      </div>
      <div class="flex flex-col items-end justify-end">
        <div class="text-1rem text-primaryColor c-text-black">${{formatAmount(erc20Balances.BNB[erc20] * prices[erc20.toLowerCase()])}}</div>
      </div>
    </div>
    ...
    <div class="flex justify-between items-center py-1rem px-1.5rem border-b-1 border-listBgBorder"
      ...
      <div class="flex items-center">
        <img class="w-2rem h-2rem rounded-full border-1px gradient-border"
             :src="icons[erc20]" alt="">
        <div class="text-left ml-1rem">
          <div class="c-text-black text-1rem">{{names[erc20]}}(Polygon)</div>
          <div class="text-text8F text-0.75rem font-bold mt-0.5rem">{{ formatAmount(erc20Balances.MATIC[erc20]) }} {{erc20}}</div>
        </div>
      </div>
      <div class="flex flex-col items-end justify-end">
        <div class="text-1rem text-primaryColor c-text-black">${{formatAmount(erc20Balances.MATIC[erc20] * prices[erc20.toLowerCase()])}}</div>
      </div>
    </div>
    ...
    <div class="flex justify-between items-center py-1rem px-1.5rem border-b-1 border-listBgBorder">
      <div class="flex items-center">
        ...
        <div class="text-left ml-1rem">
          <div class="c-text-black text-1rem">Steem</div>
          <div class="text-text8F text-0.75rem font-bold mt-0.5rem">{{ formatAmount(steemBalance) }} STEEM</div>
        </div>
      </div>
      <div class="flex flex-col items-end justify-end">
        <div class="text-1rem text-primaryColor c-text-black">{{ steemValue }}</div>
