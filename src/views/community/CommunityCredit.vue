<template>
  <div class="py-15px px-15px">
    <InfluenceCardItem
      ...
      :community="communityCCscore"
      class="cursor-pointer bg-primaryBg light:bg-color27"
    ></InfluenceCardItem>

    <div class="py-10px">
      <div
        class="c-text-black text-1.8rem mb-3rem min-h-1rem"
      ...
      <div
        v-else-if="!refreshing && (!members || members.length === 0)"
        class="py-2rem"
      >
      ...
      >
        <div
          class="flex justify-between items-center px-15px c-text-black text-14px py-10px gap-5px"
        >
          <span class="flex-1 text-left">{{ $t("community.member") }}</span>
          <span class="flex-1 text-right">{{
            $t("community.curationCredit")
          }}</span>
          <!-- <span class="flex-1 text-right">{{$t('community.communityTokenBalance')}}</span> -->
        </div>
        <div
          class="flex justify-between items-center px-15px py-10px gap-5px"
        ...
        >
          <div class="flex items-center flex-1 truncate">
            <div class="flex items-center">
              ...
              <Avatar
                :profile-img="item.profileImg || defaultAvatar"
                :name="item.name"
                ...
                @gotoUserPage="
                  $router.push({ path: '/account-info/@' + item.username })
                "
              >
                <template #avatar-img>
                  <img
                    class="w-32px min-w-32px h-32px rounded-full mr-5px border-0.5px border-color62/20"
                    @click.stop="
                      $router.push({ path: '/account-info/@' + item.username })
                    "
                    :src="item.profileImg || defaultAvatar"
                    alt=""
                  />
                </template>
              </Avatar>
...
export default {
  ...
  computed: {
    ...
    communityCCscore() {
      if (this.communityCC) {
        const cc = this.communityCC.find(
          (c) => c.communityId == this.community.communityId
        );
  ...
  async activated() {
    let count = 0;
    while (
      !this.showingCommunity ||
      !this.showingCommunity.communityId ||
      count++ < 30
    ) {
  ...
  methods: {
    parseTimestamp,
    async onLoad() {
      if (this.listFinished || this.finished || this.members.length === 0)
        return;
      try {
        this.listLoading = true;
        let members = await getCommunityMembers(
          this.showingCommunity.communityId,
          20,
          parseInt((this.members.length - 1) / 20) + 1
        );
        if (members.length > 0) {
          // members = await this.getBalances(members)
          this.$store.commit(
            "community/saveMembers",
    ...
    async getBalances(mem) {
      const balances = await getTokenBalancesOfUsers(
        EVM_CHAINS_ID[this.showingCommunity.chainId],
        this.showingCommunity.rewardToken,
        mem.map((m) => m.ethAddress)
      );
      mem = mem.map((m) => {
        let balance = balances[m.ethAddress];
        balance = balance
          ? balance.toString() / 10 ** this.showingCommunity.rewardTokenDecimals
          : 0;
